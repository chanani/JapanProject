package com.project.thejapenproject.security.filter;
...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        // accessToken 이 없거나 Bearer로 시작하지 않으면
        if (accessToken == null || accessToken.startsWith("Bearer ") == false){
            if(refreshToken == null || refreshToken.startsWith("Bearer ") == false){
        ...
        try {
            ...
            String token = accessToken.substring(7); // Bearer 공백 이후 진짜 토큰
